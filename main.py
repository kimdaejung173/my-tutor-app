...
# --- 데이터 로드 함수 (기존 로직을 Supabase로 대체) ---

def fetch_users():
    """Supabase users 테이블에서 유저 정보 가져오기"""
    if not supabase: return pd.DataFrame()
    try:
        response = supabase.table('users').select('*').execute()
        if response.data:
            # 기존 코드 호환성을 위해 DataFrame 변환 및 컬럼 타입 통일
...
def fetch_questions():
    """Supabase questions 테이블에서 문제 가져오기"""
    if not supabase: return pd.DataFrame()
    try:
...
def fetch_solved_ids(user_name):
    """Supabase study_logs 테이블에서 푼 문제 ID 가져오기"""
    if not supabase: return set()
    try:
        # 기존 코드에서 user_name을 식별자로 썼으므로 여기서도 name 혹은 id 매칭
        # (Supabase에 user_id가 있다면 그것을 쓰는게 좋으나, 백업 코드 로직 유지를 위해 name 필터링 가정)
        # 만약 users 테이블의 id를 쓴다면 user_name 변수에 id가 들어와야 함.
        response = supabase.table('study_logs').select('problem_id').eq('name', user_name).execute()
        if response.data:
...
class HomeworkApp:
    ...
    def process_login(self):
        ...
        if users_df.empty:
            ui.notify("유저 정보가 없습니다.", type='negative')
            return
    ...
    # --- 관리자 전용 화면 (백업.txt 그대로) ---
    def render_admin_page(self):
        self.main_container.clear()
        
        # 관리자 페이지 들어올 때 문제 목록 최신화
        global df
        df = fetch_questions()
    ...
    def load_new_question(self):
        ...
        # Supabase에서 푼 문제 ID 가져오기
        solved_ids = fetch_solved_ids(self.user_name)
    ...
    def render_question_page(self):
        with self.main_container:
            ...
            try:
                # 데이터베이스에 리스트형태나 ^ 구분자로 들어가 있을 수 있음. 호환성 처리
                opts_raw = str(q.get('options', ''))
                if '^' in opts_raw:
                    opts = opts_raw.split("^")
                elif opts_raw.startswith('[') and opts_raw.endswith(']'):
                    opts = json.loads(opts_raw.replace("'", '"')) # 단순 파싱 시도
                else:
                    opts = [opts_raw]

                trans_raw = str(q.get('option_trans', ''))
                if '^' in trans_raw:
                    opt_trans = trans_raw.split("^")
                else:
                    opt_trans = [trans_raw] # 혹은 빈 리스트
            except: 
                opts, opt_trans = [], []
            ...
            # translation 필드가 없거나 비어있을 수 있으므로 처리
    ...
    def check_answer(self, user_choice):
        ...
        user_num = user_choice.strip()[1] # (1) ... 형식 가정
